...
async function downloadContentfulAssets(gatsbyFunctions) {
  const {
    actions: {
      createNode,
      touchNode,
  ...
  await distributeWorkload(assetNodes.map(node => async () => {
    ...
    const url = (0, _imageHelpers.createUrl)(node.file.url); // Avoid downloading the asset again if it's been cached
